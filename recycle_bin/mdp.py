...
def main(args):
    ...
    with wandb.init(project='mastering MinAtar with world models', config=config_dict):
        ...
        last_time = time.time()  # start timing

        for iter in range(1, trainer.config.train_steps):
            # every 1000 steps, report elapsed time and reset timer
            if iter % 1000 == 0:
                elapsed = time.time() - last_time
                print(f"Time for last 1000 steps (up to iter {iter}): {elapsed:.2f} seconds")
                last_time = time.time()
            ...
            if done:
                ...
                if len(scores) > 100:
                    ...
                    if current_average > best_mean_score:
                        ...
                        print(
                            'saving best model with mean score : ',
                            best_mean_score
                        )
                ...
                prev_rssmstate = trainer.RSSM._init_rssm_state(1)
                prev_action = torch.zeros(1, trainer.action_size).to(
                    trainer.device
                )
                episode_actor_ent = []
            else:
